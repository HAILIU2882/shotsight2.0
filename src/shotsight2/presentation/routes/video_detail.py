...
from shotsight2.api.deps import get_analysis_job_service, get_video_library_service
...
def video_detail_page(
    ...
    return jinja_templates.TemplateResponse(
        request, "video_detail.html", {"locale": locale, "detail": detail, "error": None, "success": None}
    )
...
def start_analysis(
    ...
    backend_version: Annotated[str, Form()],
    ...
    try:
    ...
    except (VideoNotReadyError, ValueError) as exc:
        detail = library.get_video_detail(video_id)
        if detail is None:
        ...
        return jinja_templates.TemplateResponse(
            request,
            "video_detail.html",
            {"locale": locale, "detail": detail, "error": str(exc), "success": None},
            status_code=409,
        )
...
def reanalyze(
    ...
    return start_analysis(request, video_id, backend_name, backend_version, job_service, library, locale)
